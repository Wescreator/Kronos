...
import { Plus, CheckCircle, Trash2, TrendingDown, Tag, Edit } from 'lucide-react'
...
function CategorySelect({ categories, value, onChange, onCategoryCreated }) {
  ...
  const PRESET_COLORS = ['#7C5CFC','#34D399','#FB7185','#38BDF8','#FBBF24','#F472B6','#A78BFA','#6EE7B7']
  ...
  const handleCreate = async () => {
    ...
    } catch (err) { toast.error(err.response?.data?.message || 'Erro ao criar categoria') }
    ...
    <div>
      ...
      {showCreate && (
        <div className="mt-2 p-4 rounded-2xl"
          ...
          <p className="text-xs font-bold mb-3" style={{ color: '#A78BFA' }}>Nova categoria</p>
          <input className="input flex-1 text-sm mb-3" placeholder="Nome da categoria"
            ...
            <div className="flex gap-1.5">
              {PRESET_COLORS.map(c => (
                <button key={c} type="button" onClick={() => setNewColor(c)}
                  ...
                  style={{ background: c, outline: newColor === c ? `2px solid ${c}` : 'none', outlineOffset: 2, transform: newColor === c ? 'scale(1.2)' : 'scale(1)' }} />
              ))}
            </div>
          </div>
          <div className="flex gap-2">
            <button type="button" onClick={() => setShowCreate(false)} className="btn-secondary text-xs py-1.5 px-3">Cancelar</button>
...
export default function ExpensesPage() {
  ...
  const handleEdit = async (e) => {
    ...
    try {
      ...
      setEditItem(null); setForm(EMPTY_FORM); load()
    } catch (err) { toast.error(err.response?.data?.message || 'Erro ao editar despesa') }
  }
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      toast.success('Pagamento confirmado!'); setPayModal(null); setPayDate(''); load()
  ...
  // Totais apenas do mês selecionado
  ...
  const FORM_MODAL = (
    <form onSubmit={editItem ? handleEdit : handleCreate} className="space-y-5">
      ...
      <div className="flex justify-end gap-3 pt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <button type="button" onClick={() => { setShowNew(false); setEditItem(null); setForm(EMPTY_FORM) }} className="btn-secondary">
          Cancelar
        </button>
  ...
  return (
    <div className="fade-in">
      ...
      <div className="flex gap-2 mb-5">
        {[{value:'',label:'Todas'},{value:'pending',label:'Pendente'},{value:'paid',label:'Pago'},{value:'overdue',label:'Atrasado'}].map(f => (
              ...
              {expenses.map((e, i) => (
                <tr key={e.id}
                  ...
                  <td className="table-cell">
                    ...
                    {e.description && (
                      <p className="text-xs mt-0.5 truncate max-w-[200px]" style={{ color: 'var(--text-muted)' }}>{e.description}</p>
                    )}
                  </td>
                  ...
                  <td className="table-cell">
                    <div className="flex items-center gap-1">
                      ...
                      {e.status === 'pending' && (
                        <button onClick={() => { setPayModal(e.id); setPayDate('') }}
                          className="p-2 rounded-xl transition-all duration-150"
                          style={{ color: '#34D399' }}
