    ...
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', zIndex: 1 }}>
      ...
      <svg width="100%" height="100%" viewBox={`0 0 ${W} ${W}`}>
        ...
        {Array.from({ length: 60 }).map((_, i) => {
          ...
          return (
            <line key={i}
              ...
              strokeWidth={isHour ? 1.5 : 0.7} strokeLinecap="round"
            />
          )
        })}
        {NUMERALS.map(({ label, a }) => {
          ...
          return (
            <text key={label} x={pt.x} y={pt.y} textAnchor="middle" dominantBaseline="central"
              fill="rgba(196,181,253,0.85)" fontSize="21" fontFamily="'Plus Jakarta Sans', sans-serif"
        ...
        <g style={{ transform: `rotate(${hrDeg}deg)`, transformOrigin: `${cx}px ${cy}px` }}>
          <line x1={cx} y1={cy - 175} x2={cx} y2={cy + 42}
            stroke="rgba(167,139,250,0.50)" strokeWidth="3.5" strokeLinecap="round" />
        </g>
        <g style={{ transform: `rotate(${minDeg}deg)`, transformOrigin: `${cx}px ${cy}px` }}>
          <line x1={cx} y1={cy - 255} x2={cx} y2={cy + 52}
            stroke="rgba(167,139,250,0.62)" strokeWidth="2" strokeLinecap="round" />
        </g>
        <g style={{ transform: `rotate(${secDeg}deg)`, transformOrigin: `${cx}px ${cy}px` }}>
          <line x1={cx} y1={cy - 305} x2={cx} y2={cy + 72}
            stroke="url(#secHand)" strokeWidth="1.2" strokeLinecap="round" />
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      // Limpa ambos os storages antes do novo login
      localStorage.clear()
      ...
      // Passa o estado "remember" para o store decidir qual storage usar
    ...
    <>
      <style>{`
        ...
        .kn-forgot {
          ...
          font-size: 13px; color: rgba(124,92,252,0.85);
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: 18 }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    <input className="kn-input" type={show ? 'text' : 'password'} placeholder="••••••••"
                      value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} required />
                    <button type="button" onClick={() => setShow(!show)} style={{
                      position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)',
                      background: 'none', border: 'none', cursor: 'pointer',
                      color: 'rgba(167,139,250,0.40)', padding: 4, display: 'flex', alignItems: 'center', transition: 'color .2s',
                    }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 26 }}>
                  ...
                  {/* ← Agora navega para a página de recuperação */}
                  <button type="button" className="kn-forgot" onClick={() => navigate('/forgot-password')}>
                    Esqueci minha senha
                  </button>
                </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 26 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                stroke="rgba(124,92,252,0.55)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
